...
(function() {
  ...
  function saveSessions() {
    // Save sessions metadata (without audio blob)
  ...
  // Audio stored in IndexedDB for larger storage
  ...
  async function startRecording() {
    ...
    try {
      ...
      // UI: Show recording state (subtle)
      ...
      // If mic permission denied, still allow note-taking without audio
  ...
  async function exportSession() {
    ...
    downloadBlob(notesBlob, `${sanitizeFilename(session.title)}_notes.md`);

    // 2. Export audio if available
    try {
      ...
      if (audioData && audioData.blob) {
        ...
        downloadBlob(audioData.blob, `${sanitizeFilename(session.title)}_recording.${ext}`);
      }
    } catch (err) {
      console.warn('No audio to export:', err);
    }

    // 3. Export combined JSON for Claude analysis
    const combined = {
      session: {
        title: session.title,
    ...
    const jsonBlob = new Blob([JSON.stringify(combined, null, 2)], { type: 'application/json' });
    downloadBlob(jsonBlob, `${sanitizeFilename(session.title)}_for_analysis.json`);
  }
